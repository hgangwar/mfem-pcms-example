...
int main(int argc, char* argv[])
{
  ...
  args.AddOption(&sp_solver, "-sp", "--strumpack", "-no-sp", "--no-strumpack",
  ...
  if (!slu_solver && !sp_solver && !cpardiso_solver) {
    ...
    if (cpardiso_solver) {
      ...
      cpardiso->SetMatrixType(
        CPardisoSolver::MatType::REAL_STRUCTURE_SYMMETRIC);
  ...
  app->AddField("density", MFEMFieldAdapter(std::string("thermal_density"),
                                            *pmesh, *fespace, x));
  app->BeginSendPhase();
  ...
  if (visualization) {
    ...
    for (int i = 0; i < nev; i++) {
      ...
      mode_sock << "parallel " << num_procs << " " << myid << "\n"
                << "solution\n"
                << *pmesh << x << flush << "window_title 'Eigenmode " << i + 1
                << '/' << nev << ", Lambda = " << eigenvalues[i] << "'" << endl;

      // press q to exit. this is done so that it continues to send data to the
      // coupler
      char c = 'q';
      if (myid == 0) {
