                  ...
                  string prec_type)
{
  ...
  int kappa = 1;
  // Create mesh
  const int nx_half = nxy/2;
  Mesh mesh = Mesh::MakeCartesian2D(nx_half, nxy, Element::QUADRILATERAL, true, 0.5, 1.0);
  ...
  auto residual = support::SolveSystem(fem, solver_type, prec_type,
                                                1e-8, 500);
  // ---- Save mesh + solution (.pvd) ----
  support::SaveParaview(pmesh, *fem.x,
  "app_A",
  "Temperature",
  0, 0.0);
  /*
                  ...
                  string prec_type)
{
  ...
  const int nx_half = nxy/2;
  Mesh mesh = Mesh::MakeCartesian2D(nx_half, nxy, Element::QUADRILATERAL, true, 0.5, 1.0);
  // Shift right half by +0.5 in x so it sits in [0.5, 1.0]
  ...
  // Initialize the FEA System
  support::FEMSystem fem =
    support::Init_FEMSystem(&pmesh, order, 'B');
