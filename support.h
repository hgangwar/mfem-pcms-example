...
namespace support
{
  ...
  ------------------------------------------------------------
  */
  inline void SaveParaview(ParMesh &pmesh,
                       ParGridFunction &x,
                                          ...
                                          double tol)
  {
    ...
    for (int be = 0; be < pmesh.GetNBE(); be++)
    {
      ...
      for (int j = 0; j < verts.Size(); j++)
      {
        ...
        if (std::abs(v[0] - a) > tol) { on_plane = false; break; }
      }
      ...
      for (int k = 0; k < vdofs.Size(); k++)
      {
        ...
        if (tdof >= 0) { mark[tdof] = 1; }
      }
    }
  }

  // -----------------------------
  // Mark essential TRUE dofs on internal plane x=a via vertices (mesh-conforming only)
  // -----------------------------
  ...
  {
    ess_tdofs.SetSize(0);
    for (int i = 0; i < mark.Size(); i++) { if (mark[i]) { ess_tdofs.Append(i); } }
  }
                                              ...
                                              double T_mid)
  {
    ...
    for (int vi = 0; vi < pmesh.GetNV(); vi++)
    {
      ...
      if (!(is_left || is_right || is_mid)) { continue; }

      double val = 0.0;
      if (is_left)       val = T_left;
      else if (is_right) val = T_right;
      else               val = T_mid;
